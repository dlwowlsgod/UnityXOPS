...
namespace UnityXOPS
{
    ...
    public class FadeInFadeOut : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (clock >= fadeInEnd && clock < fadeOutStart)
            {
                _image.color = _transparentColor;
            }
            
            if (clock >= fadeOutStart && clock < fadeOutEnd)
            {
                var lerp = Mathf.InverseLerp(fadeOutStart, fadeOutEnd, clock);
                _image.color = Color.Lerp(_transparentColor, _visibleColor, lerp);
            }
