...
{
    /// <summary>
    /// 공식 미션 목록을 스크롤하며 표시하고 선택된 미션을 로드하는 컨트롤러.
    /// </summary>
    public class OfficialMissionItemController : MissionItemController
    {
        [SerializeField]
        ...
        private List<OfficialMissionData> m_data;

        private Color32 m_shadowColor;

        /// <summary>
        /// 공식 미션 데이터를 로드하고 버튼 텍스트·이벤트·스크롤 상태를 초기화한다.
        /// </summary>
        protected override void Start()
        {
            m_data = DataManager.Instance.MissionData.officialMissions;
            ...
            else
            {
                ...
                ScrollToIndex(MainmenuScene.OfficialScrollIndex);
            }
        }

        /// <summary>
        /// 지정한 인덱스로 스크롤하여 미션 버튼 텍스트와 상태를 갱신한다.
        /// </summary>
        private void ScrollToIndex(int topIndex)
        {
            topIndex = Mathf.Clamp(topIndex, 0, m_maxItemIndex);
            m_topIndex = topIndex;
            MainmenuScene.OfficialScrollIndex = topIndex;
        ...
        /// <summary>
        /// 미션 항목 클릭 시 해당 공식 미션을 로드한다.
        /// </summary>
        public override void MissionItemClicked(int index) => mainmenuScene.Load(m_topIndex + index, false);
    }
}
